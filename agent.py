...
class Ship:
    def __init__(self, unit_id: int):
        ...
        self.action: ActionType | tuple[ActionType, int, int] | None = None
    ...
    def create_actions_array(self):
        ...
        for i, ship in enumerate(ships):
            if ship.action is not None:
                # jeśli jest to samo ActionType to ustaw (action, 0, 0)
                if isinstance(ship.action, ActionType):
                    actions[i] = ship.action, 0, 0
                # jeśli tuple to (ActionType, dx, dy)
                elif isinstance(ship.action, tuple):
                    a_type, dx, dy = ship.action
                    actions[i, 0] = a_type
                    actions[i, 1] = dx
                    actions[i, 2] = dy
    ...
    def try_sap_on_enemies(self, ship) -> bool:
        ...
        for opp_ship in self.opp_fleet:
            ...
            if dist <= Global.UNIT_SAP_RANGE:
                dx = opp_ship.coordinates[0] - ship.coordinates[0]
                dy = opp_ship.coordinates[1] - ship.coordinates[1]
